...
def free_energy(matrix, beta):
    ...
    n = len(matrix)
    p_Smax = np.ones(n) / n # uniform distribution
    ...
    functionals_values = []
    functionals_values.append(U_min)
    functionals_values.append(p_Umin)
    functionals_values.append(S_max)

    return functionals_values #U_min, p_Umin, S_max, p_Smax, Free_energy
...
def plot_SA_data(entropy_history, entropy_history_averaged, energy_history, energy_history_averaged):
    ...
    if energy_history_averaged is not None:
    ...
    if entropy_history_averaged is not None:
